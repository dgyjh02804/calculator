...
class AlgebraicExpression:
    ...
    def __sub__(self, other):
        ...
        neg_terms = [term * Fraction(-1, 1) for term in other.terms]
...
def main():
    calculator = AlgebraicCalculator()
    ...
    print("代数表达式化简器")
    print("=" * 70)
    print(f"{'表达式':<25} {'期望结果':<25} {'实际结果':<25} {'状态':<10}")
    print("-" * 70)

    all_correct = True
    for expr, expected in test_cases:
        result = calculator.simplify_expression(expr)
        status = "✓" if result == expected else "✗"
        if result != expected:
            all_correct = False
            print(f"{expr:<25} {expected:<25} {result:<25} {status:<10}")
        else:
            print(f"{expr:<25} {expected:<25} {result:<25} {status:<10}")
    ...
    while True:
        try:
            ...
            result = calculator.simplify_expression(user_input)
            print(f"化简结果: {result}")
