...
class AlgebraCalculatorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("代数表达式计算器 V1.28.1  2.26")
        ...
        self.equation_categories = {
            ...
            "分式方程": [
                ...
                ("(x^2-1)/(x-1) = 0", "x = -1"),
                ("x/(x-2) = 3/(x-2)", "x = 3"),
                ("1/x + 1/(x+1) = 1", "x = 1/2+1/2√(5) 或 1/2-1/2√(5)"),
            ],
            "多变量方程": [
                ("x+y=10", "多变量方程的解:\n  x = -y+10\n  y = -x+10"),  # 修改此处
                ("(x+y)^2=0", "多变量方程的解:\n  x = -y\n  y = -x"),
                ...
                ("1/x+1/y=5/(x+y)",
                 "多变量方程的解:\n  x = (3/2)y+1/2√(5) 或 (3/2)y-1/2√(5)\n  y = (3/2)x+1/2√(5) 或 (3/2)x-1/2√(5)"),
                ("x/y + y/x = xy",
                 "多变量方程的解:\n  x = √((-y^2)/(-y^2+1)) 或 -√((-y^2)/(-y^2+1))\n  y = √((-x^2)/(-x^2+1)) 或 -√((-x^2)/(-x^2+1))"),  # 修改此处
                ("x/y + y/x = 4x^2y",
                 "多变量方程的解:\n  x = 目前不支持求解这类方程，已化简：x^2/y-4x^3y+y\n  y = √((-x^2)/(-4x^3+1)) 或 -√((-x^2)/(-4x^3+1))"),  # 修改此处
            ],
            "联立方程组": [
                ("x+y=5; x-y=1", "x = 3, y = 2"),
    ...
    def run_tests(self):
        ...
        for sub in expr_selected:
            for test_input, expected in self.expression_categories[sub]:
                all_tests.append(("表达式", test_input, expected))

        # 添加方程测试
        for sub in eq_selected:
            for test_input, expected in self.equation_categories[sub]:
    ...
    def _run_tests_thread(self, all_tests):
        ...
        try:
            ...
            for i, (test_type, test_input, expected) in enumerate(all_tests):
